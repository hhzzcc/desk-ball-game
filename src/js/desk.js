...
export class Desk {
    ...
    async initBall() {
        const ball = new Ball();
        const promises = [ball.init({ w: BALL_WIDTH, h: BALL_HEIGHT, x: 142, y: 200, vx: 0, vy: 0, url: '/src/imgs/white-ball.png', type: 'white' })];
        for (let col = 0; col < 4; col++) {
            for (let i = 0; i < col + 1; i++) {
                const ball = new Ball();
                const promise = ball.init({ w: BALL_WIDTH, h: BALL_HEIGHT, x: 400 + col * BALL_WIDTH, y: 200 + i * BALL_HEIGHT - col * BALL_HEIGHT / 2, vx: 0, vy: 0, url: '/src/imgs/yellow-ball.png' });
    ...
    async run() {
        ...
        if (v === 0) {
            if (this.balls[0].type !== 'white') {
                const ball = new Ball();
                await ball.init({ w: BALL_WIDTH, h: BALL_HEIGHT, x: 142, y: 200, vx: 0, vy: 0, url: '/src/imgs/white-ball.png', type: 'white' });
                this.draw(ball);
